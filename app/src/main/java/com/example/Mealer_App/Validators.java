...
public class Validators {
    ...
    public boolean validateAddress(EditText textStreet, EditText textStreetNum, EditText textPostal, EditText textCity) {
        return (validateName(textStreet) && validateNumber(textStreetNum) && validatePostal(textPostal) && validateName(textCity));
    }

    public boolean validateUsername(EditText textUsername, Database dbHelper) {
        String username = textUsername.getText().toString().trim();

        if(dbHelper.checkUsernameExists(username)) {
            textUsername.setError("Username already exists");
            return false;
        }
